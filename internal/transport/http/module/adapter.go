...
func NewAdapter(binding metadatasdk.Binding) (modulehttp.Adapter, error) {
	...
	if len(handlers) != 0 {
		...
		return nil, fmt.Errorf("Metadata HTTP handlers have no Action route entries: %v", keys)
	}
	return &metadataAdapter{handler: handler.mux, routes: routes}, nil
}
...
func (h *metadataHandler) listDefinitions(writer http.ResponseWriter, request *http.Request) {
	...
	values, err := h.definitions.List(request.Context(), metadatasdk.DefinitionQuery{Owner: metadatasdk.DefinitionOwnerMetadata, ResourceType: resourceType})
	if err != nil {
...
func (h *metadataHandler) getDefinition(writer http.ResponseWriter, request *http.Request) {
	...
	value, found, err := h.definitions.Get(request.Context(), metadatasdk.DefinitionOwnerMetadata, strings.TrimSpace(request.PathValue("resourceType")), strings.TrimSpace(request.PathValue("resourceKey")))
	if err != nil {
		writeMetadataError(writer, err)
		return
	}
